@extends('layouts.app')
@section('page-title', 'Sunting chatbot')
...
<div class="max-w-2xl">
    <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('chatbots.embed', $chatbot) }}" class="btn btn-secondary btn-sm">Dapatkan kod pemasangan</a>
            <a href="{{ route('knowledge.index', $chatbot) }}" class="btn btn-secondary btn-sm">Soal jawab chatbot</a>
        </div>
    </header>
    <form action="{{ route('chatbots.update', $chatbot) }}" method="POST" class="card p-5 sm:p-8 space-y-5">
        ...
        <div>
            <label for="bot_name" class="label">Nama yang dipaparkan kepada pengunjung</label>
